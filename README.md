...
WARNING 時：
  Slack 放行/拒絕按鈕 → Supabase Edge Function（驗 Slack 簽章）
       ├─ 放行：schema-gate=success + repository_dispatch(approve) → resume job 佈署 placeholder
       └─ 拒絕：schema-gate=failure（取消）
```
...
supabase/functions/slack-approval/    # Edge Function：驗章 → 設 commit status → dispatch
...
3. **取得憑證**：
   ...
   - GitHub → 產生一個**細粒度 PAT（`GH_DISPATCH_TOKEN`）**，最小權限：commit statuses (write) + repository dispatch（給 Edge Function 用）。
...
```
腳本會：把 6 個 GitHub Actions secrets 推到 repo、`supabase link`、套用 `db/schema.sql`、`supabase functions deploy slack-approval --no-verify-jwt`、設定 Edge Function 的 `SLACK_SIGNING_SECRET` 與 `GH_DISPATCH_TOKEN`。

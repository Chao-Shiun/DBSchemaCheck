...
Bitbucket PR → main
  └─ Bitbucket Pipelines
       └─ 只有 PR reviewer 含指定 DB-check bot 時才執行 ci/bitbucket-schema-review.sh
            ├─ ERROR：bot reviewer request changes，pipeline 失敗
            ├─ WARNING：bot reviewer request changes，等待 Slack 放行/拒絕
...
notepad scripts/.env.setup   # 填入 SLACK_BOT_TOKEN / SUPABASE_* / SLACK_SIGNING_SECRET / GH_DISPATCH_TOKEN；Bitbucket approval 另填 BITBUCKET_API_*
```
...
```
腳本會：把 GitHub Actions secrets 推到 repo、`supabase link`、套用 `db/schema.sql`、`supabase functions deploy slack-approval --no-verify-jwt`、設定 Edge Function 的 `SLACK_SIGNING_SECRET`、`GH_DISPATCH_TOKEN`，以及可選的 `BITBUCKET_API_USERNAME` / `BITBUCKET_API_TOKEN`。
